...
import { useProfile, listPendingProfiles, listApprovedProfiles, approveUser, rejectUser, deleteUser } from '@/lib/profile'
...
export default function DataMgmtView() {
  ...
  return (
    <div className="p-6">
      ...
      {/* Users (admins only) */}
      {isAdmin && (
        <div className={`rounded-xl border mt-6 overflow-hidden ${isLight ? 'bg-white border-black/[0.10] shadow-sm' : 'bg-[#111418] border-white/7'}`}>
          <div className={`px-5 py-4 border-b ${isLight ? 'border-black/[0.08]' : 'border-white/7'}`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-xs mt-0.5 ${isLight ? 'text-gray-500' : 'text-[#a8b0be]'}`}>
                  Approved accounts with dashboard access. Deleting a user is permanent.
                </p>
              </div>
              <button
          ...
          {approved.length === 0 ? (
            ...
            <div>
              {approved.map(u => {
                const busy = approvedBusyId === u.id
